...
class ConfigHandler(object):
        ...
        for index, account in enumerate(self.account_list):
            if "nickname" in account:
                indexed_accounts.append(
                    {
                        "names": [account["username"], account["nickname"]],
                        "index": index,
                    }
                )
            else:
                indexed_accounts.append(
                    {"names": [account["username"]], "index": index}
                )
    ...
    def select_account(self, cmd_args):
        ...
        if len(cmd_args) > 1 and self.get_account_index(cmd_args[1]) is not False:
        ...
        # if there is only 1 account in the config
            ...
            print(
                f"Only 1 account found; Using {self.get_nickname(self.account_list[0])}"
            )
        ...
        # else if there are no accounts in the config
            ...
            while True:
                ...
                if choice <= len(self.account_list) and choice >= 0:
                    return self.account_list[choice]
                else:
